...
% standard header
    ...
    \usepackage[a2paper,margin=2cm]{geometry}
...
% packages
    ...
    \usepackage{pdflscape} % https://tex.stackexchange.com/questions/150394/how-to-switch-from-portrait-to-landscape-page-orientation?noredirect=1&lq=1
% commands
    \newcommand*{\vt}{~\vert~}
...
\section{Metatheory}
    ...
    \begin{lemma}[Weakening]
        \vphantom{.}\\
        If $\J$ and $\ContextWellFormed[\HoleCtx\xExtendTypVarCtx[][L][\knd[L]]]$, then $\J[\HoleCtx\xExtendTypVarCtx[][L][\knd[L]]]$
    ...
    \begin{proof}
        ...
        \begin{mathpar}
            \infer[PK-$\lambda$]
                {\infer[Marked-Exchange]
                    {\infer[Weakening]
                        {\infer[premiss]
                            { }
                        {\PrincipalKind[\HoleCtx\xExtendTypVarCtx[][][\knd[1]]]{\typ}[\knd[2]]}
                        \\\infer[CWF-TypVar]
                            {\infer[Weakening]
                                {\infer[premiss of subderivation]
                                    {\infer[IH]
                                        { }
                                    {\ContextWellFormed[\HoleCtx\xExtendTypVarCtx[][L][\knd[L]]]}}
                                {\KindWellFormed[\knd[L]]}
                                \\\infer[COK]
                                    {\infer[premiss]
                                        { }
                                    {\PrincipalKind[\HoleCtx\xExtendTypVarCtx[][][\knd[1]]]{\typ}[\knd[2]]}}
                                {\ContextWellFormed[\HoleCtx\xExtendTypVarCtx[][][\knd[1]]]}}
                            {\KindWellFormed[\knd[L]][\HoleCtx\xExtendTypVarCtx[][][\knd[1]]]}
                            \\\infer[]
                                { }
                            {\typvar[L]\notin\xExtendTypVarCtx[][][\knd[1]]}}
                        {\ContextWellFormed[\HoleCtx\xxExtendTypVarCtx[][]{\knd[1]}[L]{\knd[L]}]}}
                    {\PrincipalKind[\HoleCtx\xxExtendTypVarCtx[][]{\knd[1]}[L]{\knd[L]}]{\typ}[\knd[2]]}
                    \\\infer[CWF-TypVar]
                        { }
                    {\ContextWellFormed[\HoleCtx\xxExtendTypVarCtx[][L]{\knd[L]}[]{\knd[1]}]}}
                {\PrincipalKind[\HoleCtx\xxExtendTypVarCtx[][L]{\knd[L]}[]{\knd[1]}]{\typ}[\knd[2]]}}
            {\PrincipalKind[\HoleCtx\xExtendTypVarCtx[][L][\knd[L]]]{\TypeFun[][\knd[1]]}[\SKind[\DepFunKind][\TypeFun[][\knd[1]]]]}
        \end{mathpar}
        \end{landscape}
        \begin{tabular*}{\textwidth}{lc@{\extracolsep{\fill}}ll}
            Weakening.
                %&PK-$\lambda$& $\PrincipalKind[\HoleCtx\xxExtendTypVarCtx[][]{\knd[1]}[L]{\knd[L]}]{\typ}[\knd[2]]$ & by Weakening on premiss \\
                && $\KindWellFormed[\knd[1]][\HoleCtx\TypVarCtx[]]$ & by subderivation premiss \\
                && $\ContextWellFormed[\HoleCtx\xExtendTypVarCtx[][L][\knd[L]]]$ & by IH \\
                && $\KindWellFormed[\knd[1]][\HoleCtx\xExtendTypVarCtx[][L][\knd[L]]]$ & by Weakening on subderivation premiss \\
                && $\ContextWellFormed[\HoleCtx\xxExtendTypVarCtx[][L]{\knd[L]}[]{\knd[1]}]$ & by CWF-TypVar \\
                && $\ContextWellFormed[\HoleCtx\xxExtendTypVarCtx[][]{\knd[1]}[L]{\knd[L]}]$ & by ? \\
                && $\PrincipalKind[\HoleCtx\xxExtendTypVarCtx[][]{\knd[1]}[L]{\knd[L]}]{\typ}[\knd[2]]$ & by Weakening on premiss \\
                && $\PrincipalKind[\HoleCtx\xxExtendTypVarCtx[][L]{\knd[L]}[]{\knd[1]}]{\typ}[\knd[2]]$ & by Marked-Exchange \\
                && $\PrincipalKind[\HoleCtx\xExtendTypVarCtx[][L][\knd[L]]]{\TypeFun[][\knd[1]]}[\SKind[\DepFunKind][\TypeFun[][\knd[1]]]]$ & by PK-$\lambda$ \\
            OK-PK.
